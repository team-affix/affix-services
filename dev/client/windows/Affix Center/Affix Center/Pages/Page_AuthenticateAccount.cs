...
namespace Affix_Center.Forms
{
    public partial class Page_AuthenticateAccount : UserControl
    {
        public Page_AuthenticateAccount()
        {
            ...
            txtPassword.textBox.KeyDown += (sender, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; Perform(); } };
        ...
        {
            App.Vals.Page_AccountUnauthenticated.ShowRegisterAccount();
        }
        ...
        void Perform()
        {
            ...
            Action GetAccessTokenSucceed = () =>
            {

            };
            Action<byte[], byte[]> RetrievedAccessToken = (encryptedAccessToken, encryptedIFAPrivateKey) =>
            {
                try
                {
            ...
            App.Vals.Account_Active.string_IdentificationEmailAddress = txtEmailAddress.textBox.Text;
            App.Methods.void_GetAccountAccessToken(GetAccessTokenFail, GetAccessTokenSucceed, RetrievedAccessToken);
        }
    }
}
